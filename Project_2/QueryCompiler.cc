...
RelationalOp* QueryCompiler::joiner(OptimizationTree* tempRoot, Schema &parentSchema, AndList* _predicate, int &_pos) {
    ...
    */
    ...
    if (tempRoot->leftChild != NULL) {
        joinROPL = joiner(tempRoot->leftChild, joinSchemaL, _predicate, posL);
    }
    //cout << "Checking Right Child" << endl;
    if (tempRoot->rightChild != NULL) {
        joinROPR = joiner(tempRoot->rightChild, joinSchemaR, _predicate, posR);
    }
    
    //cout << tempRoot->tables.size() << endl;
...
//                cout << (*x) << endl;
                ...
                // Selects Position should be 2 so join is 3
                _pos = 2;
                return (*x);
            }
        }
...
void QueryCompiler::Compile(TableList* _tables, NameList* _attsToSelect, FuncOperator* _finalFunction, AndList* _predicate, NameList* _groupingAtts, int& _distinctAtts, QueryExecutionTree& _queryTree) {
    ...
    Schema joinSchema;
    int pos;
    RelationalOp* joinOp = joiner(root, joinSchema, _predicate, pos);
